...
maxDepth(struct TreeNode* root) {
    ...
    q = enqueue(q, current_node);

    while(NULL != current_node) {
        if(NULL != current_node->tree_node->left) {
            struct QueueNode *left_node = initQueueNode(current_node->tree_node->left, 1 + depth);
            (void) printf("Node left: %d\n", left_node->tree_node->val);
            q = enqueue(q, left_node);
        }
        if(NULL != current_node->tree_node->right) {
            struct QueueNode *right_node = initQueueNode(current_node->tree_node->right, 1 + depth);            
            (void) printf("Node right: %d\n", current_node->tree_node->right->val);
            q = enqueue(q, right_node);
        }
        (void) printQueue(q);
        (void) printf("-----------\n");
        q = deque(q);
